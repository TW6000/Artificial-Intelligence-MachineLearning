...
import pandas as pd # used for managing datasets 
...
X = data.drop('NObeyesdad', axis=1)  # Where axis 0= row, axis 1= column - we need to remove this column from input data - this is our training data 
...
# Using 20% of data as training data for learning with a random selection based on 42
...
# Scale the data
# Work out the max and the mins of the data 
...
log_y_pred_class = log_model.predict(X_test) # Results of logistic regression prediction are now stored
...
tree_y_pred_class = tree_model.predict(X_test) # Results of tree based classifier prediction are now stored

 

# Confusion Matrixes generated comparing predicted test with real target data 
